  ...
  public:
    ...
    Token *WhatKindOfToken(string str, int line, int column) {
        ...
        if (str == "(")
            ...
            t->mtype = STRING;
        /*
        else if ( strcmp("quote", str.c_str() ) == 0 ) { // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        эquote!!!!!!!!!!!!! if ( DEBUG_PRINT_EVAL ) cout << "is quote!!" << endl ; t->mtype = QUOTE; } //
        else if
        */
        else
            t->mtype = SYMBOL;
    ...
    Token *PeekAToken() { return mNextToken; } // PeekAToken()
    ...
    bool IsExit(vector<Token> expr) {
        if (expr.size() == 3 && expr[0].mname == "(" && expr[1].mname == "exit" &&
            expr[2].mname == ")") //  ( exit )
            return true;
        else if (expr.size() == 5 && expr[0].mname == "(" && expr[1].mname == "exit" &&
                 expr[2].mname == "." && expr[3].mname == "nil" && expr[4].mname == ")") //  ( exit . nil)
            return true;
        else if (expr.size() == 6 && expr[0].mname == "(" && expr[1].mname == "exit" &&
                 expr[2].mname == "." && expr[3].mname == "(" && expr[4].mname == ")" &&
                 expr[5].mname == ")") //  ( exit . () )
            return true;
        return false;
    } // IsExit()
    ...
    bool IsATOM(Token *token) {
        if (token->mtype == SYMBOL || token->mtype == INT || token->mtype == FLOAT ||
            token->mtype == STRING || token->mtype == NIL || token->mtype == T)
            return true;
        return false;
    } // IsATOM()
        ...
        if (IsATOM(PeekAToken())) {
            ...
            if (IsSexp(expr, false, false)) { // 12
                ...
                while (IsSexp(expr, true, false)) {
                    ...
                    if (gerrorMsg == true || geofcheck == true)
                        return false;
                } // while
    ...
    void PrettyPrint(Token *temp) {
        ...
        */
        ...
        if (temp->mtype == QUOTE || IsATOM(temp)) { // S-expressions only an ATOM
            ...
            if (DEBUG_Print_H)
                cout << "# left NOT only ATOM #" << endl;

            if (temp->mright->mtype == NIL) {

                if (DEBUG_Print_H)
                    cout << "# left NOT only ATOM -> right==NIL #" << endl;
                ...
                if (DEBUG_Print_H)
                    cout << "# left NOT only ATOM -> right=' || right is atom #" << endl;
                ...
                if (DEBUG_Print_H)
                    cout << "# left NOT only ATOM -> right is node #" << endl;
                ...
                if (DEBUG_Print_H)
                    cout << "# NOT only ATOM -> else goto right #" << endl;
                PrettyPrint(temp->mright);
                if (DEBUG_Print_H)
                    cout << "# NOT only ATOM -> else goto right  return#" << endl;

            } // else
        } // else if
        else {
            if (DEBUG_Print_H)
                cout << "# left is nothing #" << endl;
            cout << "( ";
            ...
            if (DEBUG_Print_H)
                cout << "type: " << temp->mleft->mtype << endl; // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            gM = gM + 2;

            if (DEBUG_Print_H)
                cout << "# left is nothing -> goto left #" << endl;
            PrettyPrint(temp->mleft);
            if (DEBUG_Print_H)
                cout << "# left is nothing -> goto left return #" << endl;

            if (temp->mright->mtype == NIL) {
                if (DEBUG_Print_H)
                    cout << "# left is nothing -> right==NIL #" << endl;
                ...
                if (DEBUG_Print_H)
                    cout << "# left is nothing -> right==QUOTE || ATOM #" << endl;
                ...
                if (DEBUG_Print_H)
                    cout << "# left is nothing -> right == nothing #" << endl;

                for (int i = 0; i < gM; i++)
                    cout << " ";

                if (DEBUG_Print_H)
                    cout << "# left is nothing -> right == nothing -> goto right #" << endl;
                PrettyPrint(temp->mright);
    ...
    while (exit == false && geofcheck == false) {
        ...
        if (t.IsExit(expr) == true)
            ...
            */
            ...
            tokens = expr;
            head = t.BuildCons(
                expr, head); // !!!!!!!!!!!!!!!!!!!!!!! return head pointer セ磅︽Ч硂︽head is NULL
            DP(DEBUG, "PrettyPrint succeeded!");
...
void CheckAndGetList(Token *root, vector<Token *> &vTemp) {
    ...
    for (Token *walk = root->mright; walk != NULL; walk = walk->mright) {
        ...
        if (walk->mtype == NIL && walk->mright == NULL)
            ...
            if (DEBUG_PRINT_EVAL)
                cout << "mname: " << walk->mleft->mname << endl;
            if (DEBUG_PRINT_EVAL)
                cout << "mtype: " << walk->mleft->mtype << endl;
        } // else if
...
Token *EvaluateSExp(Token *root) {
    ...
    if (root->mtype == LEFT_PAREN) { // left is a symbol to eval
        ...
        if (DEBUG_PRINT_EVAL)
            cout << "RootLeft_mname: " << root->mleft->mname << endl;
        if (DEBUG_PRINT_EVAL)
            cout << "RootLeft_mtype: " << root->mleft->mtype << endl;
        ...
        if ( IsAlreadyDefined( root->mleft->mname, index )  ) { // check this function name is binding or not
          ...
          root->mleft->mname.assign( gSymbolTable.at( index ).bindingValue->mname ) ; // !!!!!!!! string
        assign
        ...
        */
        ...
        if (IsAFunction(root->mleft)) {
            ...
            if (DEBUG_PRINT_EVAL)
                cout << "left is quote" << endl;
            if (DEBUG_PRINT_EVAL)
                cout << "quote 1st element: " << root->mright->mleft->mleft->mname << endl;
        ...
        // else if ( strcmp( root->mleft->mname.c_str(), "(" ) == 0 && root->mIsBeEvaluate == false ) {
            ...
            // No Name Function
            if (DEBUG_PRINT_EVAL_P2)
                cout << "No Name Function" << endl;
            if (DEBUG_PRINT_EVAL_P2)
                cout << "function Name: " << root->mleft->mname << endl;
            root->mleft = EvaluateSExp(root->mleft);

            if (DEBUG_PRINT_EVAL_P2)
                cout << "No Name Fun return " << endl;
            ...
            if ((t.IsATOM(root->mleft) == true && root->mleft->mtype != SYMBOL) || isBeReplacedOnce) {
                ...
                // No Name Function
                if (DEBUG_PRINT_EVAL_P2)
                    cout << "No Name Function attempt to apply non-function";
                ...
                if (DEBUG_PRINT_EVAL_P2)
                    cout << " else: " << root->mleft->mname << endl;
        ...
        if (DEBUG_PRINT_EVAL_P2)
            cout << "symbol unbound: " << root->mleft->mname << endl;
        // ERROR
...
bool IsAFunction(Token *temp) { // quote 矪瞶
    for (int i = 0; i < FunctionCount; i++) {
        if ((strcmp(temp->mname.c_str(), gFunctionList[i].c_str()) == 0) && (temp->mtype != QUOTE)) {
            ...
            if (DEBUG_PRINT_EVAL)
                cout << "Func Name: " << gFunctionList[i].c_str() << endl;
            return true;
        } // if
    } // for
...
Token *IsAtom(vector<Token *> args) { // atom?
    ...
    if (args.size() != 1) {
        // ERROR
        if (DEBUG_PRINT_EVAL_P2)
            cout << "IsAtom args size : " << args.size() << endl;
    ...
    if (args.size() != 1) {
        ...
        if (args.at(0)->mtype == SYMBOL || args.at(0)->mtype == INT || args.at(0)->mtype == FLOAT ||
            ...
            for (Token *walk = args.at(0); walk != NULL; walk = walk->mright) { // ﹍ぃノ琧攫
                // walk  "." ênode
                if (DEBUG_PRINT_EVAL_P2)
                    cout << " walk->mtype: " << walk->mtype << endl;
                if (DEBUG_PRINT_EVAL_P2 && (walk->mright == NULL))
                    cout << " walk->mright = NULL" << endl;

                if (walk->mtype == NIL && walk->mright == NULL)
                    walk->mright =
                        NULL; // 竒┏  磷耞Αerror // The use of 'break' is not allowed here.
...
Token *And(vector<Token *> args) {
    ...
    if (args.size() < 2) {
        ...
        for (int i = 0; i < args.size(); i++) {
            ...
            if (DEBUG_PRINT_EVAL_P2)
                cout << "total: " << args.size() << "and at: " << i << endl;
...
Token *EqualTo(vector<Token *> args) { // =
    ...
    if (args.size() < 2) {
        ...
        for (int i = 1; i < args.size(); i++) { // compare
            ...
            if (DEBUG_PRINT_EVAL_P2)
                cout << "a - b: " << firstNum - otherNum << endl;
    ...
    if (args.size() < 2) {
        ...
        for (int i = 1; i < args.size(); i++) { // compare
            ...
            if (strcmp(firstStr.c_str(), tempStr.c_str()) == 0 ||
                ...
                if (DEBUG_PRINT_EVAL_P2)
                    cout << "onceSmllerThanOrEqual" << endl;
...
Token *If(Token *root, vector<Token *> args) {
    ...
    if (args.size() != 2 && args.size() != 3) {
        if (DEBUG_PRINT_EVAL_P2)
            cout << "If args size: " << args.size() << endl;
    ...
    */
    ...
    if (args.empty()) {
        ...
        for (int i = 0; i < args.size(); i++) {
            ...
            if (DEBUG_PRINT_EVAL_P2)
                cout << " before size: " << tempArgs.size();
            CheckAndGetSubList(args.at(i), tempArgs);
            argsList.push_back(tempArgs);

            if (DEBUG_PRINT_EVAL_P2)
                cout << "      after size: " << tempArgs.size() << endl;
        ...
        */
        for (int i = 0; i < argsList.size(); i++) {
            ...
            if (i ==
                argsList.size() - 1) { // 程 condition is else ( else ぃ材)->ヘ玡⊿耞硂
                ...
                */
                ...
                /*
                if ( i == argsList.size() - 1  ) { // 程 condition is else    ( else
                ぃ材)->ヘ玡⊿耞硂
